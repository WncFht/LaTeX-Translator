...
- `-o, --output-dir <目录>`: 输出目录 (可选，使用配置文件中的值)
...
翻译过程会生成以下文件（保存在指定的输出目录中）：

- `{原文件名}_masked.txt`: 掩码后的文本
- `{原文件名}_masked_map.json`: 掩码节点映射
- `{原文件名}_translated.txt`: 翻译后但未替换的文本
- `{原文件名}_translated.tex`: 最终翻译后的LaTeX文件
